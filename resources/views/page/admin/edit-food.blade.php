...
@section('content')
    <div class="grid grid-cols-12 bg-[#F9FAFB]">
        <form method="POST" action="/admin/food/edit/{{$food->id}}" class="col-start-4 col-end-10 my-4" enctype="multipart/form-data">
            @csrf
            ...
            <div class="p-4 flex flex-col bg-[#F9FAFB] rounded-lg border border-[#E5E7EB] mb-2">
                ...
                <input class="bg-[#F9FAFB]" type="number" id="price" name="price" value="{{$food->price}}" placeholder="5" style="outline: none" required>
            </div>
            @error('price')
                <span class="text-red-600">
                    {{ $message }}
